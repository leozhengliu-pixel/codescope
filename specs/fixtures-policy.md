...
| Search corpus for API search behavior | `crates/test_support/repo_tree_fixture.rs` for the shared canonical temp root; `crates/search/src/lib.rs` (`LocalSearchStore::seeded()`, test-only `create_test_store()`) for search-only ignored/binary variants | Mix of seeded live-workspace repo mapping plus synthetic temp-dir files for focused search tests | Allowed when sourced from local rewrite repo or rewrite-authored temp files | Reuse the shared repo-tree root helper before adding another temp root builder; keep search-only ignored/binary variants in the search test wrapper until a later task04b2 slice centralizes them |
| Browse / blob / glob / grep repo trees | `crates/test_support/repo_tree_fixture.rs` for the shared canonical temp root; `crates/api/src/browse.rs` (`LocalBrowseStore::seeded()`, test-only `create_test_store()`) for browse-only symlink variants | Rewrite-owned temp directories, files, and symlink edge cases generated in tests | Allowed: generated entirely inside this repo/test runtime | Prefer the shared repo-tree root helper for common README/src/target layout and keep browse-only symlink edge cases in the browse test wrapper until a later task04b2 slice centralizes them |
...
- **Shared repo-tree root helper:** `crates/test_support/repo_tree_fixture.rs`
- **Browse temp corpora and symlink variants:** `crates/api/src/browse.rs`
- **Commit/git temp corpora:** `crates/api/src/commits.rs`
- **Search temp corpora and ignored/binary variants:** `crates/search/src/lib.rs`
