...
- Repository-sync jobs tied to a configured `local` connection now run a timeout-bounded real Git preflight (`git -C <repo_path> rev-parse --is-inside-work-tree`) before falling back to stub outcomes: missing, invalid, or hung local repositories fail closed with an operator-visible `local repository sync preflight failed` error, while a real local Git working tree can complete successfully even when the configured stub outcome is `failed`.
