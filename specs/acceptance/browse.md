...
10. Diff view renders additions/deletions and handles renamed/copied files; backend diff-file patch text is bounded to 64 KiB per file and returns `patch_truncated: true` plus an inline truncation marker when that cap is reached, and the UI renders that state as an explicit truncation notice instead of displaying the marker as if it were the complete patch.
