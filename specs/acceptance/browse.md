...
2. Repository detail shows key metadata, default branch, sync/index status, a bounded authenticated branch/tag summary section from `/api/v1/repos/{repo_id}/refs`, and a branch/tag/revision control that can keep the repo route stable while reloading browse and commit panels for the selected revision. The visible refs section is summary-only and must not be claimed as full branch picker or revision-switching parity.
...
## Black-box examples
- Opening a repo page shows repository metadata, default branch, latest sync state, bounded branch/tag summaries, and browse/commit panels; applying a branch/tag/revision updates the route plus reloads tree/blob and commit data for that selected revision without losing the surrounding repo shell, and commit history previous/next controls request the backend-provided offsets from `page_info`. The branch/tag summary list is visibility only and remains separate from full branch picker/navigation parity.
