...
8. Commit list is ordered consistently and supports pagination with explicit `limit`/`offset` request controls plus response `page_info` (`limit`, `offset`, `has_next_page`, `next_offset`) so clients can render truthful history navigation; for a visible repository with a successful local sync job and no seeded commit-store history, commit list/detail/diff reads may fall back to the latest caller-authorized local Git working tree.
