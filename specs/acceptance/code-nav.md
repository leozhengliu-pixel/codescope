...
2. References lookup returns a deduplicated, navigable list of usages; the bounded text-reference scanner covers Rust plus TypeScript/JavaScript source extensions (`.ts`, `.tsx`, `.js`, `.jsx`, `.mts`, `.cts`, `.mjs`, `.cjs`) so references parity matches the declaration extractor's supported language set, and revision-backed scans skip common generated/dependency directories (`.git`, `target`, `node_modules`, `dist`) before loading candidate blobs.
