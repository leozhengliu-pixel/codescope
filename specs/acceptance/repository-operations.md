...
- `/api/v1/repos/{repo_id}/index-status` returns the search store's current startup-built local in-memory index status for visible repositories, including `indexed`, `indexed_empty`, or `error` state, indexed file count, indexed line count, skipped file count, and error text when startup indexing failed for that repo; if the startup search store has no status for that repository and the latest authorized local sync has a `search-index.json` artifact, the endpoint reports the artifact-backed counts even when the caller can see the same repository through multiple organizations, or an `error` status for a malformed artifact. Any present startup status remains authoritative, including startup `error` states, so local-sync fallback cannot mask an active primary indexing failure. The artifact writer now also refuses to persist serialized `search-index.json` payloads above the same bounded loader size cap. If no artifact exists, the pre-artifact snapshot fallback is used only when the snapshot path is a real non-symlink directory; a symlinked snapshot is treated as no fallback and returns `404` rather than traversing the link target.
...
7. The repository index-status endpoint reports the bounded search-store baseline or, when the startup store has no status for that repo, the latest authorized local sync `search-index.json` artifact/pre-artifact snapshot fallback across any caller-authorized organization for that repository, with malformed/non-regular/error-status artifacts failing closed as an `error` status with zero counts and a bounded/redacted reason rather than request-wide 500s or mutable snapshot rewalks, not catalog `sync_state`: `indexed` for repositories with searchable files, `indexed_empty` for successfully scanned repositories with zero searchable files, or `error` with indexed file count, indexed line count, skipped file count, and optional error text. Any present startup status remains authoritative, including startup `error`, so local-sync fallback cannot hide an active primary indexing failure.
...
- A configured `DATABASE_URL`, bounded catalog reads, the startup-built in-memory index-status endpoint, the local sync artifact-backed requested-repo search/index-status including requested-repo primary search-error fallback only when a valid authorized local-sync artifact can answer and index-status fallback only when startup status is absent, unscoped/search-context search fallback including stale/error primary-index bypass for newer authorized artifacts, snapshot tree/blob/code-navigation fallback, bounded branch/tag ref summaries, and local-Git commit list/detail/diff fallback scoped to the caller-authorized `(organization_id, repository_id)` boundary, and the manual failed-job retry endpoint do not by themselves prove repository-operations parity; persisted index-status, background reindexing, queue depth, automated retry/backoff, failure-recovery, frontend branch-management controls, and operator-control surfaces remain open.
...
- An authenticated user calling `/api/v1/repos/{repo_id}/index-status` for a visible repository receives the current startup-built in-memory index state/counts, including `indexed_empty` for a successfully scanned repository with no searchable files; only when the startup store has no status for that repo may the endpoint use the latest authorized local sync `search-index.json` artifact counts, while malformed artifacts fail closed as an error status without widening to mutable snapshot contents and startup `error` status remains visible rather than being masked by fallback; the same call for a hidden repository returns `404`.
...
3. **Frontend repository-operations parity is still shallow.** The current UI shows coarse sync-state badges/fields, repository-detail index-status counts, bounded branch/tag summary visibility, read-only history, and bounded commit-history previous/next pagination controls, but not full branch picker/revision-switching parity, last-run timestamps, queue depth, failure history management, retry controls, or settings-level operator surfaces.
