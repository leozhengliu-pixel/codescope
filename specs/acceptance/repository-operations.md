...
7. The repository index-status endpoint reports the bounded search-store baseline or, when the startup store has no status, the latest authorized local sync `search-index.json` artifact/pre-artifact snapshot fallback, not catalog `sync_state`: `indexed`/`error`, indexed file count, indexed line count, skipped file count, and optional error text.
