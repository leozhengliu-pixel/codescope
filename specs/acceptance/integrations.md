...
5. MCP server exposes authenticated HTTP/API manifest, tool-list, and tool-call surfaces under `/api/v1/mcp/...`, plus an authenticated JSON-RPC MCP endpoint at `/api/v1/mcp` for `initialize`, `ping`, `tools/list`, and `tools/call`; repository-aware tool calls run under the caller's permission scope. MCP HTTP/API and JSON-RPC responses require well-formed JSON-compatible `Accept` negotiation: omitted/wildcard/application-JSON values are accepted, while unacceptable or malformed quality values fail closed with `406`. The JSON-RPC endpoint requires JSON request bodies, returns JSON-RPC `-32600` protocol errors for invalid request ids instead of echoing non-conformant id values, and rejects `initialize` requests that advertise an unsupported `protocolVersion` with `-32602` instead of silently completing the handshake. `ping` returns an empty JSON-RPC result object for client liveness checks without requiring repository storage access. Retrieval tool-list schemas advertise closed JSON objects (`additionalProperties: false`) matching runtime unknown-field rejection. Retrieval calls such as `read_file` fail closed before storage lookup when the active repository is outside the caller repository scope and return a structured MCP `permission_denied` error; the current local browse retrieval baseline skips obvious build-artifact directories (`.git`, `target`, `node_modules`, `dist`) during repo-scoped recursive file discovery.
