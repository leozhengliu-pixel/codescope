...
| Operator | Confirm service liveness, bounded metadata readiness, and frontend bootstrap config | `/healthz`; `/readyz`; `/api/v1/config` | `specs/acceptance/operator-runtime.md` | None for the current local liveness/readiness/config baseline; extend the existing spec before claiming broader deployment/runtime behavior |
| Operator | Run the default one-tick worker or an explicit bounded multi-tick loop against organization state path wiring | `crates/worker/src/main.rs` `main`; `AppConfig::from_env()`; `build_organization_store(config.organization_state_path.clone())`; `SOURCEBOT_WORKER_MAX_TICKS`; `SOURCEBOT_WORKER_IDLE_SLEEP_MS`; `run_worker_tick(...)`; startup runtime-baseline log fields for resolved path, stub selections, tick count, and idle sleep | `specs/acceptance/worker-runtime.md` | None for the current one-tick/default plus bounded-loop stub baseline; extend `specs/acceptance/worker-runtime.md` before broader worker orchestration work proceeds |
...
| Operator | Run the local operator maintenance baseline | `scripts/backup_local_runtime_state.sh`; `scripts/restore_local_runtime_state.sh`; `scripts/backup_local_metadata_db.sh`; `scripts/restore_local_metadata_db.sh`; Makefile `runtime-backup`/`runtime-restore`/`metadata-backup`/`metadata-restore`; README local maintenance runbook | `specs/acceptance/operator-maintenance.md` | None for the current file-backed runtime backup/restore plus local Postgres metadata backup/restore and SQLx maintenance baseline; extend `specs/acceptance/operator-maintenance.md` before claiming broader durable-surface or production deployment parity |
...
2. **Worker runtime** now has a dedicated home in `specs/acceptance/worker-runtime.md` for the one-tick default, explicit bounded multi-tick loop, startup runtime-baseline logging, optional local status snapshots, stubbed terminal outcomes, bounded completion logging, and explicit no-work logging. That baseline still does not claim retries, production scheduling/supervision, or full automation orchestration.
3. **Local end-to-end smoke matrix** now has a real repo-local command in `scripts/check_end_to_end_smoke_matrix.sh` that bootstraps auth and then drives authenticated connections, search, ask, public review-webhook intake, and default one-tick worker completion against an isolated temp runtime. That baseline is intentionally stub-backed and bounded rather than a release-certification matrix.
4. **Operator maintenance baseline** now has a dedicated acceptance home in `specs/acceptance/operator-maintenance.md` for the shipped file-backed runtime backup/restore helpers, manifest-validated runtime restore target checks, a local Postgres metadata backup/restore baseline, and the current local SQLx migration/upgrade runbook, without over-claiming that every runtime surface is durable yet.
5. **Review webhook intake and run inspection** spans both operator and admin concerns; the current authenticated/public route set should still be moved into a dedicated review-automation acceptance doc before broader implementation.
6. **Repository operations** now has a dedicated acceptance home in `specs/acceptance/repository-operations.md`; later sync/index parity work should extend that doc instead of overloading `browse.md`.
