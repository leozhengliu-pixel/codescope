...
3. **Local end-to-end smoke matrix** now has a real repo-local command in `scripts/check_end_to_end_smoke_matrix.sh` that bootstraps auth and then drives authenticated connections, search, ask, public review-webhook intake, and one-shot worker completion against an isolated temp runtime. That baseline is intentionally stub-backed and bounded rather than a release-certification matrix.
