...
6. Authenticated users can create, list, select, and delete saved search contexts through `/api/v1/auth/search-contexts`; the dedicated `#/search` route exposes a bounded management baseline for caller-owned contexts, and `GET /api/v1/search?context_id=...` applies the saved context as an additional repository-scope filter.
