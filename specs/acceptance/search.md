...
- Persisted local-sync search index artifacts must fail closed when the artifact path is not a direct regular file (including symlinks), when their embedded `index_status.repo_id` differs from the repository id requested by the loader, when `index_status.indexed_line_count` disagrees with the stored indexed-line payload, when `index_status.indexed_file_count` undercounts the unique stored result paths, when `indexed` status reports zero indexed files, when `indexed_empty` status reports non-zero indexed files or lines, when `error` status still carries indexed files/lines/content or lacks a non-empty operator-visible error message, when `indexed`/`indexed_empty` success status carries a stale error message, when any stored result path is absolute, escapes via `..`, contains blank path components, uses a non-normal separator form, or targets a runtime/generated/binary path that the live indexer would skip (for example `.sourcebot`, `target`, `node_modules`, `dist`, or skipped extensions), when any stored result has line number zero, when stored path/line keys are duplicate or not sorted by path then line number, or when stored line numbers for one path are non-contiguous; corrupted artifacts are not trusted for search result paths or repository status identity/counts. Artifact-backed search fails closed for present malformed or non-regular artifacts instead of following indirection, falling back to mutable snapshot contents, returning an empty success for the affected repo, or silently skipping the bad artifact into partial unscoped/search-context success for other visible repositories, while index-status surfaces malformed or non-regular artifacts as an `error` status with a redacted `malformed local-sync search index artifact` parse/load reason. Search treats a primary startup index with `error` status as unavailable for fallback purposes, so a visible requested repository can still use its latest authorized local-sync artifact instead of returning an empty success from the failed primary index. Cross-repository search may merge caller-authorized local-sync artifact results for visible repositories missing from the primary startup index without suppressing primary-index hits from other visible repositories.
- This saved-context, pagination, and local search-matching baseline now includes API metadata plus bounded `#/search` previous/next controls for result pages, frontend boolean/literal/regex mode selection with deep-link preservation, authenticated listing, creation from a specific non-empty repository filter, selection, and deletion of caller-owned saved search contexts on `#/search` that sends non-blank `context_id` values to `/api/v1/search`, explicit blank-`repo_id` fail-closed behavior, authenticated `400 Bad Request` handling for empty or over-1024-byte search queries, explicit `mode=boolean|literal|regex` matching, fail-closed local regex matching for empty/whitespace-only patterns, over-1024-byte patterns, and regex programs over the configured compiled-size limit, repository-root-relative skipping of `.sourcebot` runtime metadata so local-sync artifacts are not indexed as source files, case-insensitive boolean text matching, all-term matching for unquoted space-separated query terms within boolean groups, quoted-phrase components, a bounded unquoted `OR` separator between line-term groups, minimal unquoted `lang:` filters plus substring and glob-style `path:` result-path filters, fail-closed persisted-artifact non-regular/symlink path, symlinked temp-artifact write rejection without clobbering external targets, status repo-id, indexed-line-count, indexed-file-under-count, indexed-without-files state, indexed-empty state, error-status-with-content validation, error-status-without-operator-message validation, success-status-with-stale-error validation, zero-line-number validation, duplicate/unsorted/non-contiguous/blank-component path/line-key validation, ignored runtime/generated/binary persisted-path validation, and deterministic repo/path/line ordering for identical indexed content in the startup/local-sync search store plus merged primary/local-sync fallback API results before pagination; saved contexts remain file-backed in the organization aggregate, all-repository/empty saved-context scopes still fail closed rather than widening search, and this does not yet claim SQL-backed context durability, richer grammar beyond this bounded literal/regex/phrase/filter/OR/order support, full relevance tuning, or full stable pagination parity across changing indexes/revisions.
