...
| Worker execution parity | `specs/acceptance/worker-runtime.md` | Present in task80b | Covers the shipped one-tick default plus explicit bounded multi-tick, stub-oriented worker runtime baseline, runtime-baseline logging, no-work exits, bounded completion logs, and stub terminal outcomes while explicitly deferring real execution, retries, supervision, durable metadata, and production observability. |
| Operator workflows parity | `specs/acceptance/operator-runtime.md` | Present in task80a; extended in task89/task99 | Covers the shipped local runtime liveness/config baseline, bounded metadata readiness via `/readyz`, authenticated worker-status snapshot visibility via `/api/v1/auth/worker-status`, shared `SOURCEBOT_DATA_DIR` path wiring, and `make api` plus default one-tick/explicit bounded multi-tick `make worker` bring-up while explicitly deferring complete migration automation, richer dependency readiness, supervision, full durable metadata parity, and production-grade observability. |
...
| MCP runtime parity | `specs/acceptance/mcp-runtime.md` | Present in task107 slice | Covers authenticated JSON-RPC-over-HTTP initialize/tools/list/tools/call behavior, batch request handling, initialized notifications, and permission-scoped repository retrieval while explicitly deferring durable MCP session registration and non-HTTP transports. |
...
| --- | --- | --- | --- |
| Health, readiness, public config, and worker status | `/healthz`, `/readyz`, `/api/v1/config`, `/api/v1/auth/worker-status` | Operator-visible service liveness, bounded metadata readiness, frontend bootstrap config, and authenticated visibility into the optional supervisor-readable worker status snapshot | `specs/acceptance/operator-runtime.md` |
...
| Ask/chat frontend | `web/src/App.tsx` → `AskPage` + `ChatPage` + `AgentsPage`, routes `#/ask`, `#/chat`, and `#/agents` | Dedicated ask/chat-route parity for repo-scoped prompts, authenticated repo-scoped thread history/reopen, inline citations, active-thread continuity, backend thread title/visibility metadata updates, and a dedicated operator-visible review-agent route baseline while delete controls, frontend rename/visibility controls, and richer agents-management/retry/orchestration remain follow-up work | `specs/acceptance/ask.md` |
...
| Surface family | Rewrite evidence | Parity intent | Current acceptance home |
| --- | --- | --- | --- |
| Default one-tick / bounded worker entrypoint | `crates/worker/src/main.rs` → `main`, `run_worker_tick` | Worker parity starts from an entrypoint that loads config, constructs the organization store, logs the resolved runtime baseline, invokes one worker tick by default or an explicitly bounded multi-tick loop, and logs terminal runs, no-work ticks, and bounded runtime completion | `specs/acceptance/worker-runtime.md` |
...
| Local end-to-end smoke matrix baseline | `scripts/check_end_to_end_smoke_matrix.sh` starts the real API against an isolated temp runtime, bootstraps/login-checks a local admin, exercises authenticated connections/search/ask plus public review-webhook intake, runs the real default one-tick worker with a stubbed completed outcome, and verifies the queued run reaches `completed` | Gives operators one bounded repo-local smoke command across auth, integrations, search, ask, and review-agent surfaces without claiming full release certification | `specs/acceptance/operator-runtime.md` until a dedicated review-automation acceptance doc lands |
| Repo sync/index visibility | Repo `sync_state` exposed through API/UI surfaces in `crates/api/src/main.rs` and `web/src/App.tsx`, plus authenticated sync-job history and bounded admin enqueue/retry in `/api/v1/auth/repository-sync-jobs` | Operator/user parity for sync readiness, failure visibility, read-only sync-job history, and narrow admin queued/retry controls for already visible repositories | `specs/acceptance/repository-operations.md` |
