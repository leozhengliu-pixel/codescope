...
| Operator maintenance parity | `specs/acceptance/operator-maintenance.md` | Present in task81a | Covers the shipped local runtime backup/restore helpers plus the current local SQLx migration and upgrade runbook while explicitly deferring durable metadata backup/restore parity and production deployment guarantees. |
...
| Durable metadata / migrations / backup-restore | `scripts/backup_local_runtime_state.sh`, `scripts/restore_local_runtime_state.sh`, `Makefile` runtime-backup/runtime-restore, and the current local SQLx workflow in `README.md` | Local operator maintenance parity starts with file-backed runtime backup/restore plus the repo-local migration/upgrade runbook, while durable metadata parity remains deferred | `specs/acceptance/operator-maintenance.md` |
