...
| Repo sync/index visibility | Repo `sync_state` exposed through API/UI surfaces in `crates/api/src/main.rs` and `web/src/App.tsx`, plus authenticated sync-job history and bounded admin enqueue in `/api/v1/auth/repository-sync-jobs` | Operator/user parity for sync readiness, failure visibility, read-only sync-job history, and a narrow admin queued-job control for already visible repositories | `specs/acceptance/repository-operations.md` |
