...
| Local end-to-end smoke matrix baseline | `scripts/check_end_to_end_smoke_matrix.sh` starts the real API against an isolated temp runtime, bootstraps/login-checks a local admin, exercises authenticated connections/search/ask plus public review-webhook intake, runs the real one-shot worker with a stubbed completed outcome, and verifies the queued run reaches `completed` | Gives operators one bounded repo-local smoke command across auth, integrations, search, ask, and review-agent surfaces without claiming full release certification | `specs/acceptance/operator-runtime.md` until a dedicated review-automation acceptance doc lands |
