...
5. JSON-RPC params-shape/schema errors and malformed repository-scope bindings, including non-object MCP method `params` and non-string `repo_id` values, return JSON-RPC `-32602` errors instead of panics, transport-level failures, or partial tool execution; per-tool argument validation errors that pass schema binding still return MCP-shaped `isError: true` tool results.
