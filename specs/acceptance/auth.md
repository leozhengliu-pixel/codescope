...
- When DATABASE_URL is configured, API-key inventory/create/revoke, API-key bearer auth, OAuth-client inventory/create, invite redemption, and members invite creation/cancellation/member-role update/member removal now restore from PostgreSQL-backed durable metadata. The local member invite create/redeem/cancel, member-role update, and member-removal paths also write durable PostgreSQL audit events and `/api/v1/auth/audit-events` reads those events for organizations visible to the current user, while file-backed audit visibility remains the fallback/compatibility mode. A fresh API process can still restore `/api/v1/auth/me`, `/api/v1/auth/members`, `/api/v1/auth/linked-accounts`, `/api/v1/auth/api-keys`, `/api/v1/auth/oauth-clients`, and those invite/member-lifecycle audit events even if the configured bootstrap/local-session/organization-state files are empty or stale; connections, analytics, broad audit analytics/filtering/export, OIDC/provider login and callback exchange, external provider identity events, retention/export policy, repo-permission durability, and broader whole-aggregate organization state remain follow-up work.
...
- An authenticated user can open `#/settings/linked-accounts`, load the current local account identity, any persisted external provider identities linked to that same local account, and visible organization memberships from `/api/v1/auth/linked-accounts`, confirm the route fails closed when the local session lacks a real local-account record, and see explicit copy that external provider linking/callback exchange, account merge, and SSO login remain follow-up work.
- An authenticated user can open `#/settings/oauth-clients`, load the admin-visible OAuth client inventory from `/api/v1/auth/oauth-clients`, submit a minimal authenticated create request with organization, name, and HTTP(S) redirect URIs, receive the returned plaintext `client_secret` exactly once in a focused success callout, and confirm that no secret hash or plaintext secret material is persisted into the inventory while unsafe redirect URI schemes fail closed without creating a client. The PostgreSQL-backed client metadata remains durable across API restarts when `DATABASE_URL` is configured and richer OAuth authorization/token/revocation workflows plus broader manage UX remain follow-up work.
