...
- An authenticated local-session user who administers at least one organization can open `#/settings/members`, load the admin-visible organization member and invite inventory from `/api/v1/auth/members`, inspect joined member account details plus truthful accepted-vs-pending invite status, create a minimal pending local invite through `POST /api/v1/auth/members/invites`, cancel a pending invite through `DELETE /api/v1/auth/members/invites/{invite_id}`, update a visible member role through `PATCH /api/v1/auth/members/{user_id}/role`, and remove a visible active member through `DELETE /api/v1/auth/members/{user_id}` with `organization_id` for an administered organization. The baseline records a local redemption invite in the file-backed fallback, creates/deletes durable PostgreSQL invite rows when `DATABASE_URL` is configured, updates member roles or deletes membership rows in file-backed or PostgreSQL auth metadata, rejects accepted/hidden invite/member targets without mutation, never deletes the local account when removing membership, and keeps shipped audit visibility in the file-backed audit aggregate consumed by audit APIs with `auth.member.removed` metadata. This remains bounded: no email delivery, invite resend workflow, durable audit-event storage, last-admin/self-removal policy enforcement, broad org/member CRUD, or full org/member lifecycle parity is claimed.
