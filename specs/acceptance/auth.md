...
- An authenticated local-session user who administers at least one organization can open `#/settings/members`, load the admin-visible organization member and invite inventory from `/api/v1/auth/members`, inspect joined member account details plus truthful accepted-vs-pending invite status, and create a minimal pending local invite through `POST /api/v1/auth/members/invites` for an administered organization. The create baseline records a local redemption invite in the file-backed fallback and a durable PostgreSQL invite row when `DATABASE_URL` is configured; shipped audit visibility remains in the file-backed audit aggregate consumed by audit APIs. This remains bounded: no email delivery, resend/cancel controls, role editing, member removal, or full org/member lifecycle parity is claimed.
