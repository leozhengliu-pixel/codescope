...
- `web/src/App.tsx` fetches authenticated `/api/v1/auth/repository-sync-jobs` alongside the settings connection inventory, renders one compact latest-sync summary per connection with the shared status-badge presentation plus read-only sync history newest-first by `queued_at`, keeps multiple queued/running in-progress rows on the same connection in that same newest-first order, keeps those same-card in-progress rows' queued/started/finished-or-fallback timestamp details truthful per row while preserving newest-first ordering, keeps those same-card latest-sync summaries truthful when queued and running rows share the same newest `queued_at` timestamp and arrive in reverse API order, and now deterministically prefers the running row when the same connection's queued and running rows tie on both newest `queued_at` and activity timestamp so the latest-sync summary remains truthful instead of depending on API order, including when those exact-timestamp tied same-card queued/running rows also arrive in reverse API order, when those exact-timestamp reverse-order same-card rows also reuse the same repository id, when those same-card exact-timestamp shared-repository-id rows already arrive newest-first, when opposite mixed terminal-state rows on the same card share the same newest `queued_at`, already arrive newest-first, and also reuse the same repository id, when those same-card exact-timestamp newest-first shared-repository-id rows still keep each queued/started/finished fallback timestamp detail truthful per row, and when those same-card opposite mixed terminal-state newest-first shared-repository-id rows still keep each queued/started/finished timestamp detail truthful per row and each row's error-detail presentation truthful, and keeps those same-card queued/running sync-history timestamp details deterministic and truthful when rows share both newest `queued_at` and activity timestamps in reverse API order, keeps sibling authenticated connection cards' latest-sync summaries truthful when queued and running rows sha... [truncated]
