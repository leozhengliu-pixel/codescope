...
- `web/src/App.tsx` fetches authenticated `/api/v1/auth/repository-sync-jobs` alongside the settings connection inventory, renders read-only sync history per connection newest-first by `queued_at`, and lets known sync-history rows quick-open the existing `#/repos/:repoId` detail route without implying new discovery or sync controls.
