...
- `web/src/App.tsx` fetches authenticated `/api/v1/auth/repository-sync-jobs` alongside the settings connection inventory, renders one compact latest-sync summary per connection with the shared status-badge presentation plus read-only sync history newest-first by `queued_at`, keeps multiple queued/running in-progress rows on the same connection in that same newest-first order, keeps those same-card in-progress rows' queued/started/finished-or-fallback timestamp details truthful per row while preserving newest-first ordering, keeps those same-card latest-sync summaries truthful when queued and running rows share the same newest `queued_at` timestamp and arrive in reverse API order, and now deterministically prefers the running row when the same connection's queued and running rows tie on both newest `queued_at` and activity timestamp so the latest-sync summary remains truthful instead of depending on API order, including when those exact-timestamp tied same-card queued/running rows also arrive in reverse API order, when those exact-timestamp reverse-order same-card rows also reuse the same repository id, when those same-card exact-timestamp shared-repository-id rows already arrive newest-first, when opposite mixed terminal-state rows on the same card share the same newest `queued_at`, already arrive newest-first, and also reuse the same repository id, and when those same-card exact-timestamp newest-first shared-repository-id rows still keep each queued/started/finished fallback timestamp detail truthful per row, and keeps those same-card queued/running sync-history timestamp details deterministic and truthful when rows share both newest `queued_at` and activity timestamps in reverse API order, keeps sibling authenticated connection cards' latest-sync summaries truthful when queued and running rows share that same newest `queued_at` timestamp across cards and arrive in reverse API order, still keeps each sibling card's latest-sync summary scoped and truthful when those cross-card queued/running rows also tie on both newest `queued_at` and activity timestamp, including when those exact-timestamp cross-card ties arrive in reverse API order and when those exact-timestamp newest-first sibling rows also reuse the same repository id, and now keeps those sibling cards' queued/running sync-history timestamp details deterministic and truthful when those newest cross-card rows tie on both `queued_at` and activity timestamp, including the reverse-order API case, the reverse-order shared-repository-id case, and the newest-first shared-repository-id case, without yet claiming broader operator/retry parity.`
- `web/src/App.test.tsx` covers supported connection kinds including `generic_git` and `local`, the broader settings-shell connection management flows, focused local import UX states including success, quick repo-detail navigation, per-card failure, in-flight disabling, and reset-after-edit behavior, plus the truthful generic-host quick-open affordance and its fail-closed rejection of unsafe `javascript:` URLs, along with focused per-connection sync-history coverage for loading, failure, empty, same-card multi-row in-progress timestamp details, same-card latest-sync summary truthfulness when queued and running rows share the same newest `queued_at` timestamp and arrive in reverse API order, including when those same-card exact-timestamp queued/running rows also reuse the same repository id and when those same-card exact-timestamp shared-repository-id rows already arrive newest-first, same-card queued/running timestamp-detail determinism when those newest rows also share both `queued_at` and activity timestamp in reverse API order, including when those exact-timestamp newest-first same-card rows also reuse the same repository id, sibling latest-sync summary truthfulness when queued and running rows share the same newest `queued_at` timestamp across cards and arrive in reverse API order, sibling latest-sync summary truthfulness when those cross-card queued/running rows also share both newest `queued_at` and activity timestamp, including the reverse-order API case and the shared-repository-id reverse-order exact-timestamp sibling case, queued/running isolation, sibling in-progress timestamp details, sibling queued/running timestamp-detail determinism when those newest cross-card rows tie on both `queued_at` and activity timestamp in reverse API order, including when those exact-timestamp reverse-order sibling rows also reuse the same repository id and when those exact-timestamp sibling rows already arrive newest-first while also reusing the same repository id, sibling terminal-state coexistence, same-card multi-row terminal-state shared-badge presentation, same-card multi-row terminal-state timestamp details, same-card multi-row terminal-state error details, same-card identical terminal-state error-string isolation, same-card latest-sync summary truthfulness when identical failed terminal-state error strings are present, same-card latest-sync summary truthfulness when older failed terminal-state rows are followed by a newer succeeded terminal-state row, same-card latest-sync summary truthfulness when older succeeded terminal-state rows are followed by a newer failed terminal-state row, same-card latest-sync summary truthfulness when opposite mixed terminal-state rows share the same newest `queued_at` timestamp and arrive in reverse API order, including when those tied newest rows also reuse the same repository id and when those tied newest rows already arrive newest-first while also reusing the same repository id, sibling multi-row termin...
