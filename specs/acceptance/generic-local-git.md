...
- `web/src/App.test.tsx` covers supported connection kinds including `generic_git` and `local`, the broader settings-shell connection management flows, focused local import UX states including success, quick repo-detail navigation, per-card failure, in-flight disabling, and reset-after-edit behavior, plus the truthful generic-host quick-open affordance and its fail-closed rejection of unsafe `javascript:` URLs, along with focused per-connection sync-history coverage for loading, failure, empty, same-card and sibling-card in-progress timestamp details, same-card and sibling-card mixed terminal-state latest-sync summary truthfulness including both the exact-timestamp reverse-order shared-repository-id case and the matching newest-first shared-repository-id exact-timestamp case, same-card mixed terminal-state sync-history timestamp/error details under reverse-order and newest-first exact-timestamp shared-repository-id ties, and sibling mixed terminal-state sync-history timestamp/error-detail truthfulness for the exact-timestamp reverse-order shared-repository-id case.
