...
- When the rewrite exposes connection records, settings-shell CRUD, one minimal settings-driven local import form for existing `local` connections, a single authenticated local-path import baseline, a bounded admin-only repository-sync enqueue API for already visible repositories, and a one-shot worker smoke that advances an enqueued job into authenticated terminal history, this spec records that truthfully while still deferring broader local mirror enumeration, real sync execution, retries, and durable catalog parity to later slices.
...
- A bounded admin-only repository-sync enqueue contract for already visible generic/local repositories plus a repo-local worker smoke proving that a queued job becomes authenticated terminal sync-history progress under the current stub worker baseline
...
- `crates/api/src/main.rs` now accepts authenticated admin-only `POST /api/v1/auth/repository-sync-jobs` for an already visible `(organization_id, repository_id)`, derives the job's `connection_id` from catalog repository detail, persists a queued repository-sync job, and fails closed for non-admin callers without mutating sync-job state.
...
7. An authenticated organization admin can enqueue a queued repository-sync job for an already visible repository; the repo-local smoke matrix proves that the current one-shot worker can advance that job into authenticated `succeeded` sync-history progress, but this remains a bounded stub execution baseline rather than real sync execution, retry, or background orchestration parity.
...
- Repository-sync enqueue must fail closed for non-admin users, hidden repositories, blank payload fields, or catalog-missing repositories, and non-admin failures must not append queued jobs.
...
- Manual sync enqueue can create a queued job for an already visible repository, and the current one-shot worker smoke can advance that stub job to terminal authenticated history, but it is not evidence that a provider worker can discover repositories, mirror/fetch changes, retry failures, or maintain durable index status.
...
3. **Per-connection sync/index parity is still shallow.** The settings shell now includes a minimal local-only explicit-path import form, read-only sync history, a backend-only admin enqueue baseline for already visible repositories, and a one-shot stub worker progress smoke, but not full operator controls, real sync execution, retries, import progress, generic-host discovery, recursive local enumeration, or durable per-connection indexing status parity.
