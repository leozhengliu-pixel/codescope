...
- When the rewrite exposes connection records, settings-shell CRUD, one minimal settings-driven local import form for existing `local` connections, a single authenticated local-path import baseline, and a bounded admin-only repository-sync enqueue API for already visible repositories, this spec records that truthfully while still deferring broader local mirror enumeration, real sync execution, retries, and durable catalog parity to later slices.
...
- A bounded admin-only repository-sync enqueue contract for already visible generic/local repositories
...
7. An authenticated organization admin can enqueue a queued repository-sync job for an already visible repository, but that control is only a bounded queueing baseline and does not by itself prove real sync execution, retry, or background orchestration parity.
8. The rewrite must not claim generic/local Git parity solely because the models, settings shell, one minimal settings-driven local import UX, one local import baseline, read-only sync history, and bounded manual sync enqueue exist; broader host enumeration, sync execution, and durable catalog-backed visibility still have to be proven.
...
- Manual sync enqueue can create a queued job for an already visible repository, but it is not evidence that a provider worker can discover repositories, mirror/fetch changes, retry failures, or maintain durable index status.
...
- An authenticated organization admin can post `{"organization_id":"org_acme","repository_id":"repo_visible"}` to `/api/v1/auth/repository-sync-jobs` and receive a `queued` repository-sync job for the repository's catalog connection; a viewer posting the same payload receives a fail-closed response and no persisted job.
...
3. **Per-connection sync/index parity is still shallow.** The settings shell now includes a minimal local-only explicit-path import form, read-only sync history, and a backend-only admin enqueue baseline for already visible repositories, but not full operator controls, real sync execution, retries, import progress, generic-host discovery, recursive local enumeration, or durable per-connection indexing status parity.
