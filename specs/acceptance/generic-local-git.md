...
- `web/src/App.tsx` fetches authenticated `/api/v1/auth/repository-sync-jobs` alongside the settings connection inventory, renders one compact latest-sync summary per connection with the shared status-badge presentation plus read-only sync history newest-first by `queued_at`, keeps multiple queued/running in-progress rows on the same connection in that same newest-first order, keeps those same-card in-progress rows' queued/started/finished-or-fallback timestamp details truthful per row while preserving newest-first ordering, keeps those same-card latest-sync summaries truthful when queued and running rows share the same newest `queued_at` timestamp and arrive in reverse API order, and now deterministically prefers the running row when the same connection's queued and running rows tie on both newest `queued_at` and activity timestamp so the latest-sync summary remains truthful instead of depending on API order, including when those exact-timestamp tied same-card queued/running rows also arrive in reverse API order, when those exact-timestamp reverse-order same-card rows also reuse the same repository id, when those same-card exact-timestamp shared-repository-id rows already arrive newest-first, when opposite mixed terminal-state rows on the same card share the same newest `queued_at`, arrive in reverse API order, and also reuse the same repository id, when those opposite mixed terminal-state rows on the same card instead already arrive newest-first and also reuse the same repository id, when those same-card opposite mixed terminal-state rows also share the same activity timestamp while already arriving newest-first and reusing the same repository id, when those same-card opposite mixed terminal-state rows instead share both newest `queued_at` and activity timestamps while reusing the same repository id and arriving in reverse API order, when those same-card exact-timestamp newest-first shared-repository-id rows still keep each queued/started/finished fallback timestamp detail truthful per row, and when those same-card opposite mixed terminal-state reverse-order or newest-first shared-repository-id rows still keep each queued/started/finished timestamp detail truthful per row, including when those newest same-card mixed terminal-state rows also share the same activity timestamp, when those newest same-card mixed terminal-state rows instead share both newest `queued_at` and activity timestamps while arriving in reverse API order and also reuse the same repository id, and each row's error-detail presentation truthful, and keeps those same-card queued/running sync-history timestamp details deterministic and truthful when rows share both newest `queued_at` and activity timestamps in reverse API order, keeps sibling authenticated connection cards' latest-sync summaries truthful when queued and running rows share both newest `queued_at` and activity timestamps across cards, including the reverse-order API case, the exact-timestamp shared-repository-id reverse-order cross-card case, and the exact-timestamp shared-repository-id newest-first cross-card case, and keeps sibling authenticated connection cards' queued/running sync-history timestamp details deterministic and truthful across both the reverse-order and newest-first shared-repository-id exact-timestamp cases as well as sibling mixed terminal-state latest-sync summary truthfulness when opposite newest terminal-state rows also share the same activity timestamp while already arriving newest-first and reusing the same repository id across cards, plus sibling mixed terminal-state latest-sync summary and sync-history error-detail truthfulness when opposite newest terminal-state rows already arrive newest-first and also reuse the same repository id across cards.
- `web/src/App.test.tsx` covers supported connection kinds including `generic_git` and `local`, the broader settings-shell connection management flows, focused local import UX states including success, quick repo-detail navigation, per-card failure, in-flight disabling, and reset-after-edit behavior, plus the truthful generic-host quick-open affordance and its fail-closed rejection of unsafe `javascript:` URLs, along with focused per-connection sync-history coverage for loading, failure, empty, same-card multi-row in-progress timestamp details, same-card latest-sync summary truthfulness when queued and running rows share the same newest `queued_at` timestamp and arrive in reverse API order, including when those same-card exact-timestamp queued/running rows also reuse the same repository id and when those same-card exact-timestamp shared-repository-id rows already arrive newest-first, plus same-card mixed terminal-state latest-sync summary truthfulness when those newest shared-repository-id rows also share the same activity timestamp while already arriving newest-first, and when those same-card newest shared-repository-id rows instead share both newest `queued_at` and activity timestamps while arriving in reverse API order, plus same-card mixed terminal-state sync-history timestamp-detail truthfulness when those newest shared-repository-id rows arrive in reverse API order or already arrive newest-first, including when those exact-timestamp newest-first same-card rows also reuse the same repository id, when those exact-timestamp reverse-order same-card rows also reuse the same repository id, and when those newest same-card mixed terminal-state rows also share the same activity timestamp, plus same-card mixed terminal-state sync-history error-detail truthfulness when those newest shared-repository-id rows arrive in reverse API order or already arrive newest-first, including when those newest same-card mixed terminal-state rows also share the same activity timestamp while already arriving newest-first, and when those newest same-card mixed terminal-state rows instead share both newest `queued_at` and activity timestamps while arriving in reverse API order and also reuse the same repository id, plus sibling mixed terminal-state sync-history timestamp-detail truthfulness when those newest shared-repository-id rows also share the same activity timestamp while already arriving newest-first across cards, plus sibling mixed terminal-state sync-history error-detail truthfulness when those newest shared-repository-id rows also share the same activity timestamp while already arriving newest-first across cards, sibling latest-sync summary truthfulness when queued and running rows share the same newest `queued_at` timestamp across cards and arrive in reverse API order, sibling latest-sync summary truthfulness when those cross-card queued/running rows also share both newest `queued_at` and activity timestamp, including the reverse-order API case and the shared-repository-id reverse-order exact-timestamp sibling case, queued/running isolation, sibling in-progress timestamp details, sibling queued/running timestamp-detail determinism when those newest shared-repository-id exact-timestamp rows arrive either in reverse API order or already newest-first, plus same-card and sibling mixed terminal-state latest-sync summary and sync-history error-detail truthfulness when the newest shared-repository-id rows already arrive newest-first, including the sibling latest-sync-summary case where those opposite newest terminal-state rows also share the same activity timestamp across cards.
