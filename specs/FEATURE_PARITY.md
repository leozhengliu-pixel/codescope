...
| Integrations | Generic Git host / local Git | Partial | `specs/acceptance/generic-local-git.md` now grounds the current baseline: shared `generic_git`/`local` connection kinds and configs, authenticated `/api/v1/auth/connections` CRUD, the first authenticated `POST /api/v1/auth/repositories/import/local` baseline for importing one real local Git working tree into the current catalog path, repo-detail connection metadata, and the limited `#/settings/connections` shell with local `repo_path` handling, local-only explicit-path import UX for existing local connections, quick navigation from successful imports into `#/repos/:repoId`, and read-only sync-history visibility. Generic-host discovery, broader status/control UX, and durable catalog-backed parity remain open. |
