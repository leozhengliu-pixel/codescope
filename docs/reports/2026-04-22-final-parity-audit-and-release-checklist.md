...
| Code navigation: definitions and references | Partial | Code: definitions/references endpoints in `crates/api/src/main.rs`; UI: browse panel symbol navigation in `web/src/App.tsx`; docs/tests: `specs/acceptance/code-nav.md`, `web/src/App.test.tsx`. | Works as a bounded baseline, and no-revision requests can now fall back to the latest caller-authorized local sync snapshot with terminal sync revision links when the primary browse/code-navigation store has no blob; multi-language correctness, stale-index status, real provider fetch/import/reindex, durable/background indexing, and richer UX parity remain deferred. |
...
| Sync state and indexing status | Partial | Code: repo `sync_state`, `GET/POST /api/v1/auth/repository-sync-jobs`, bounded `POST /api/v1/auth/repository-sync-jobs/{job_id}/retry`, repository-sync terminal worker logs with job/repo/connection identifiers plus failure reasons, stale-running repository-sync lease recovery before the next claim, explicit bounded multi-tick processing for multiple queued jobs in one invocation, optional supervisor-readable worker status snapshots exposed through authenticated `GET /api/v1/auth/worker-status` with last tick/outcome, and a timeout-bounded local-Git preflight plus revision/content-discovery/current-branch probes that persist terminal revision, branch, and tracked-content file-count metadata, write a bounded tracked-content manifest, materialize a sibling snapshot tree from tracked `HEAD` file bytes, persist a sibling `search-index.json` artifact that can be loaded without rewalking the snapshot before completing jobs tied to configured `local` connections, and let requested visible-repo search, unscoped/search-context search across visible repositories, and index-status load the persisted local sync `search-index.json` artifact when present, and let no-revision tree/blob reads fall back to the latest successful local sync snapshot under the caller-authorized `(organization_id, repository_id)` boundary when the startup search/browse stores have no rows/tree/blob/status for that repo; UI/docs/tests: `specs/acceptance/repository-operations.md`, `specs/acceptance/worker-runtime.md`, `web/src/App.tsx` (including local manifest/snapshot artifact paths in sync history), gap-report repository-operations rows, and focused local-sync artifact-backed search/index-status plus snapshot browse/code-navigation regressions in `crates/api/src/main.rs`. | Coarse sync visibility, backend-only queued-job creation/retry, richer worker terminal logs, default one-tick worker progress plus explicitly bounded multi-tick coverage, optional local status snapshots, stale-running lease recovery, local-Git working-tree preflight plus persisted HEAD/content-discovery/current-branch status/manifest/snapshot handling, and bounded requested-repo artifact-backed search/index-status plus snapshot tree/blob/code-navigation fallback exists, but truthful persisted/background index-status, real provider fetch/import/reindex worker execution, automated retry/backoff/progress beyond bounded manual retry plus stale-running lease recovery, and durable repository-operations parity remain blocked. |
...
| Generic Git host / local Git | Partial | Code: generic/local connection CRUD, a local import baseline with PostgreSQL catalog handoff, admin-organization visibility binding, and queued repository-sync handoff for one explicitly requested local Git working tree, bounded admin-only repository-sync enqueue/retry with active duplicate suppression in `crates/api/src/main.rs`, repo detail connection metadata in `crates/core/src/lib.rs`, timeout-bounded worker-side local-Git preflight plus persisted revision/content-count/current-branch probes, bounded tracked-content manifests, sibling tracked-`HEAD` snapshots, persisted sibling search-index artifacts for queued jobs tied to configured `local` connections, stale-running repository-sync lease recovery before the next claim, and requested visible-repo search/index-status/tree/blob/code-navigation plus unscoped/search-context search fallback and commit list/detail/diff fallback over the latest successful local sync artifact/snapshot or local Git working tree under the caller-authorized `(organization_id, repository_id)` boundary; UI/docs/tests: `#/settings/connections` (including local manifest/snapshot artifact paths in sync history), `specs/acceptance/generic-local-git.md`, `web/src/App.test.tsx`, focused PostgreSQL catalog import, focused repository-sync/worker tests including explicit bounded multi-tick processing, and the local-sync artifact-backed requested-repo search/index-status, unscoped/context search, and snapshot search/browse/code-navigation regressions. | One bounded local-import/settings/backend-enqueue/retry/local-content-discovery manifest/snapshot/metadata/search baseline exists, but generic-host discovery, recursive import, real provider fetch/import/reindex execution, automated retries/backoff beyond bounded manual retry plus stale-running lease recovery and one stale-lease replay, supervised runtime status beyond the optional local status snapshot, durable/background indexing, and full durable provider/runtime parity remain open. |
...
| MCP server | Partial | Code: `crates/mcp/src/lib.rs` ships the manifest and repository-aware retrieval tool definitions plus execution support; `crates/api/src/main.rs` now exposes authenticated HTTP/API MCP manifest, tool-list, and tool-call routes under `/api/v1/mcp/...`, with focused tests for authentication and permission-scoped `list_repos`; docs: `specs/acceptance/integrations.md`, `specs/FEATURE_PARITY.md`, integrations gap report. | Full MCP protocol/session transport interoperability, client integration, richer tool coverage, and broader end-to-end replacement evidence remain open. |
