...
| Chat threads, history, rename, visibility | Partial | Code: `/api/v1/ask/threads` and `/api/v1/ask/threads/{thread_id}` in `crates/api/src/main.rs`; UI: `#/chat` in `web/src/App.tsx`; docs/tests: `specs/acceptance/ask.md`, `web/src/App.test.tsx`. | Thread list/detail/reopen exists, but rename/delete/visibility lifecycle is still explicitly deferred. |
| Search contexts / saved scopes | Partial | Code: `/api/v1/auth/search-contexts` exists in `crates/api/src/main.rs`; docs: `specs/acceptance/index.md` and `specs/acceptance/search.md` mark this surface as present but still needing expansion; gap report API-key/admin row calls search-context behavior incomplete. | No full saved-context lifecycle or dedicated acceptance evidence yet. |
...
| Organizations, membership, invites, roles | Partial | Code/models: org/member/invite/role state in `crates/models/src/lib.rs` and auth handlers in `crates/api/src/main.rs`; UI/docs/tests: `#/settings/members`, invite-redeem flow, `specs/acceptance/auth.md`, auth/admin gap-report org row. | Read-only inventory plus invite redemption exists, but CRUD/admin management and role lifecycle parity are still missing. |
| API keys | Partial | Code: `/api/v1/auth/api-keys` plus revoke in `crates/api/src/main.rs`; UI: `#/settings/api-keys` in `web/src/App.tsx`; docs/tests: `specs/acceptance/auth.md`, `specs/acceptance/settings-navigation.md`, `web/src/App.test.tsx`. | Inventory/revoke baseline exists, but full create/scope/manage lifecycle parity is still incomplete. |
...
- [ ] Ask/chat parity is complete, including thread rename/delete/visibility lifecycle.
