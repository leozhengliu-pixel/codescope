...
| Sync state and indexing status | Partial | Code: repo `sync_state`, `GET/POST /api/v1/auth/repository-sync-jobs`, bounded `POST /api/v1/auth/repository-sync-jobs/{job_id}/retry`, and repository-sync terminal worker logs with job/repo/connection identifiers plus failure reasons; UI/docs/tests: `specs/acceptance/repository-operations.md`, `specs/acceptance/worker-runtime.md`, `web/src/App.tsx`, gap-report repository-operations rows. | Coarse sync visibility, backend-only queued-job creation/retry, richer one-shot stub worker terminal logs, and one-shot stub worker progress exist, but truthful persisted index-status, real worker execution, automated retry/backoff/progress, and durable repository-operations parity remain blocked. |
...
| Generic Git host / local Git | Partial | Code: generic/local connection CRUD, a local import baseline, and bounded admin-only repository-sync enqueue/retry in `crates/api/src/main.rs` plus repo detail connection metadata in `crates/core/src/lib.rs`; UI/docs/tests: `#/settings/connections`, `specs/acceptance/generic-local-git.md`, `web/src/App.test.tsx`, and focused repository-sync route tests. | One bounded local-import/settings/backend-enqueue/retry baseline exists, but generic-host discovery, recursive import, real sync execution, automated retries/backoff, and durable parity remain open. |
