...
| Connection management | Partial | Code: authenticated connection CRUD plus bounded admin-only repository-sync enqueue in `crates/api/src/main.rs`; UI: `#/settings/connections` in `web/src/App.tsx`; docs/tests: `specs/acceptance/integrations.md`, `specs/acceptance/generic-local-git.md`, `web/src/App.test.tsx`. | Current shell and backend enqueue baseline are still limited and do not yet prove provider-specific auth, discovery, real sync execution, or durable catalog/runtime parity. |
| Sync state and indexing status | Partial | Code: repo `sync_state` and `GET/POST /api/v1/auth/repository-sync-jobs`; UI/docs/tests: `specs/acceptance/repository-operations.md`, `web/src/App.tsx`, gap-report repository-operations rows. | Coarse sync visibility and backend-only queued-job creation exist, but truthful persisted index-status, worker execution/retry/progress, and durable repository-operations parity remain blocked. |
...
| Generic Git host / local Git | Partial | Code: generic/local connection CRUD, a local import baseline, and bounded admin-only repository-sync enqueue in `crates/api/src/main.rs` plus repo detail connection metadata in `crates/core/src/lib.rs`; UI/docs/tests: `#/settings/connections`, `specs/acceptance/generic-local-git.md`, `web/src/App.test.tsx`. | One bounded local-import/settings/backend-enqueue baseline exists, but generic-host discovery, recursive import, real sync execution/retries, and durable parity remain open. |
