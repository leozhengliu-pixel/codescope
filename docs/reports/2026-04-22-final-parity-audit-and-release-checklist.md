...
| Chat threads, history, rename, visibility | Partial | Code: `/api/v1/ask/threads` plus `GET`/`PATCH /api/v1/ask/threads/{thread_id}` in `crates/api/src/main.rs`; UI: `#/chat` in `web/src/App.tsx`; docs/tests: `specs/acceptance/ask.md`, `web/src/App.test.tsx`. | Thread list/detail/reopen, backend title/private-shared visibility updates, and focused frontend rename/visibility controls now exist, but delete/archive lifecycle, richer conversation/source-preview UX, streaming/progress states, and full chat parity remain explicitly deferred. |
