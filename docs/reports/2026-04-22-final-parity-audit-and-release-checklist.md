...
| Organizations, membership, invites, roles | Partial | Code/models: org/member/invite/role state in `crates/models/src/lib.rs` and auth handlers in `crates/api/src/main.rs`; UI/docs/tests: `#/settings/members`, minimal admin invite-create/cancel/member-role update, invite-redeem flow, `specs/acceptance/auth.md`, auth/admin gap-report org row. | Admin-visible inventory, bounded pending-invite creation/cancellation, bounded member-role updates, durable PostgreSQL invite-create/cancel/member-role storage, file-backed audit visibility, and invite redemption exist, but email delivery/resend, CRUD/admin management, member removal, and fuller role lifecycle parity are still missing. |
