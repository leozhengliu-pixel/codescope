...
| Chat threads, history, rename, visibility | Partial | Code: `/api/v1/ask/threads` plus `GET`/`PATCH /api/v1/ask/threads/{thread_id}` in `crates/api/src/main.rs`; UI: `#/chat` in `web/src/App.tsx`; docs/tests: `specs/acceptance/ask.md`, `web/src/App.test.tsx`. | Thread list/detail/reopen and backend title/private-shared visibility updates exist, but delete plus frontend rename/visibility controls are still explicitly deferred. |
