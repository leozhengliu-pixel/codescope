...
fn build_router(
    ...
    Router::new()
        ...
        .route(
            ...
            get(list_authenticated_repository_sync_jobs),
        )
        .route(
        ...
        .route(
            "/api/v1/auth/review-webhooks/{webhook_id}",
        ...
        .route(
            "/api/v1/auth/review-webhooks/{webhook_id}/delivery-attempts/{attempt_id}",
            get(get_authenticated_review_webhook_delivery_attempt_for_webhook),
        )
        .route(
            "/api/v1/auth/review-webhooks/{webhook_id}/review-agent-runs",
            get(list_authenticated_review_agent_runs_for_webhook),
        )
        .route(
            "/api/v1/auth/review-webhooks/{webhook_id}/review-agent-runs/{run_id}",
            get(get_authenticated_review_agent_run_for_webhook),
        )
        .route(
        ...
        .route(
            "/api/v1/auth/review-webhook-delivery-attempts/{attempt_id}",
            get(get_authenticated_review_webhook_delivery_attempt),
        )
        .route(
        ...
        .route(
            "/api/v1/auth/review-agent-runs/{run_id}",
            get(get_authenticated_review_agent_run),
        )
        .route(
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq)]
struct RepositorySyncJobListItemResponse {
    id: String,
    organization_id: String,
    repository_id: String,
    connection_id: String,
    status: sourcebot_models::RepositorySyncJobStatus,
    queued_at: String,
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq)]
struct OAuthClientListItemResponse {
    id: String,
    organization_id: String,
    name: String,
    client_id: String,
...
#[derive(Debug, Serialize, Deserialize, PartialEq, Eq)]
struct ReviewWebhookDeliveryAttemptListItemResponse {
    id: String,
    webhook_id: String,
    connection_id: String,
    repository_id: String,
...
async fn list_authenticated_connections(
    State(state): State<AppState>,
    headers: HeaderMap,
...
async fn update_authenticated_connection(
    State(state): State<AppState>,
    ...
    let payload: CreateConnectionRequest =
    ...
    if updated_connection.name.is_empty() {
        return Err(StatusCode::BAD_REQUEST);
    }

    organization_state.connections[existing_connection_index] = updated_connection.clone();
    state
        .organization_store
        .store_organization_state(organization_state)
        .await
...
async fn list_authenticated_repository_sync_jobs(
    State(state): State<AppState>,
    headers: HeaderMap,
    ...
    let mut jobs: Vec<_> = job_state
        ...
        .into_iter()
        .filter(|job| {
            visible_organization_ids.contains(&job.organization_id)
                && visible_repository_bindings
                    .contains(&(job.organization_id.clone(), job.repository_id.clone()))
        })
        .collect();
...
async fn list_authenticated_oauth_clients(
    State(state): State<AppState>,
    headers: HeaderMap,
...
async fn get_authenticated_review_webhook(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(webhook_id): Path<String>,
) -> Result<Json<ReviewWebhookListItemResponse>, StatusCode> {
    let session = authenticate_local_session_record(&state, &headers).await?;
    let organization_state = state
        .organization_store
        .organization_state()
    ...
    let webhook = organization_state
        .review_webhooks
        .into_iter()
        .find(|webhook| {
...
async fn get_authenticated_review_webhook_delivery_attempt_for_webhook(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path((webhook_id, attempt_id)): Path<(String, String)>,
) -> Result<Json<ReviewWebhookDeliveryAttemptListItemResponse>, StatusCode> {
    let session = authenticate_local_session_record(&state, &headers).await?;
    let organization_state = state
        .organization_store
        .organization_state()
    ...
    let attempt = organization_state
        .review_webhook_delivery_attempts
        .into_iter()
        .find(|attempt| attempt.id == attempt_id && attempt.webhook_id == visible_webhook.id)
        .ok_or(StatusCode::NOT_FOUND)?;

    Ok(Json(review_webhook_delivery_attempt_list_item_response(
        attempt,
    )))
}

async fn list_authenticated_review_agent_runs_for_webhook(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path(webhook_id): Path<String>,
    ...
    Ok(Json(
        organization_state
            ...
            .into_iter()
            .filter(|run| {
                run.webhook_id == visible_webhook.id
                    && run.organization_id == visible_webhook.organization_id
            })
...
async fn get_authenticated_review_agent_run_for_webhook(
    State(state): State<AppState>,
    headers: HeaderMap,
    Path((webhook_id, run_id)): Path<(String, String)>,
) -> Result<Json<ReviewAgentRunListItemResponse>, StatusCode> {
    let session = authenticate_local_session_record(&state, &headers).await?;
    let organization_state = state
        .organization_store
        .organization_state()
...
async fn list_authenticated_review_webhook_delivery_attempts(
    ...
    let visible_organization_ids =
        visible_organization_ids_for_user(&organization_state, &session.user_id);
    let visible_webhook_ids: HashSet<String> = organization_state
        .review_webhooks
        .iter()
        .filter(|webhook| visible_organization_ids.contains(&webhook.organization_id))
    ...
    Ok(Json(
        organization_state
            .review_webhook_delivery_attempts
            .into_iter()
            .filter(|attempt| visible_webhook_ids.contains(&attempt.webhook_id))
            .map(review_webhook_delivery_attempt_list_item_response)
            .collect(),
    ))
}

async fn get_authenticated_review_webhook_delivery_attempt(
    State(state): State<AppState>,
    headers: HeaderMap,
    ...
    let attempt = organization_state
        ...
        .into_iter()
        .find(|attempt| {
            attempt.id == attempt_id && visible_webhook_ids.contains(&attempt.webhook_id)
        })
        .ok_or(StatusCode::NOT_FOUND)?;
...
async fn get_authenticated_review_agent_run(
    State(state): State<AppState>,
    headers: HeaderMap,
...
fn oauth_client_list_item_response(client: OAuthClient) -> OAuthClientListItemResponse {
...
fn review_webhook_delivery_attempt_list_item_response(
    attempt: ReviewWebhookDeliveryAttempt,
) -> ReviewWebhookDeliveryAttemptListItemResponse {
    ReviewWebhookDeliveryAttemptListItemResponse {
        id: attempt.id,
...
fn visible_repository_bindings_for_user(
    organization_state: &OrganizationState,
    user_id: &str,
...
mod tests {
    ...
    #[derive(Debug, Deserialize)]
    struct RepositorySyncJobListItemResponseBody {
        id: String,
        organization_id: String,
        repository_id: String,
        connection_id: String,
    ...
    #[derive(Debug, Deserialize)]
    struct OAuthClientListItemResponseBody {
        id: String,
        organization_id: String,
        name: String,
        client_id: String,
    ...
    #[derive(Debug, Deserialize)]
    struct ReviewWebhookDeliveryAttemptListItemResponseBody {
        id: String,
        webhook_id: String,
        connection_id: String,
        repository_id: String,
    ...
    impl OrganizationStore for FailingOrganizationStore {
        ...
        async fn claim_next_review_agent_run(
            ...
            let mut state = self.state.lock().unwrap();
            Ok(sourcebot_core::claim_next_review_agent_run(&mut state))
        }
    ...
    #[tokio::test]
    async fn auth_repository_sync_jobs_list_only_jobs_visible_to_current_user_newest_first() {
        let organization_state_path = unique_test_path("auth-repository-sync-jobs-orgs");
        let local_session_state_path = unique_test_path("auth-repository-sync-jobs-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
            ...
            repository_sync_jobs: vec![
                ...
                sourcebot_models::RepositorySyncJob {
                    ...
                    organization_id: "org_acme".into(),
                    repository_id: "repo_hidden_stale".into(),
                    connection_id: "conn_hidden_stale".into(),
                    status: sourcebot_models::RepositorySyncJobStatus::Running,
                    queued_at: "2026-04-21T00:12:00Z".into(),
    ...
    #[tokio::test]
    async fn auth_repository_sync_jobs_do_not_leak_cross_org_duplicate_repository_ids() {
        let organization_state_path =
            unique_test_path("auth-repository-sync-jobs-cross-org-duplicate-orgs");
        let local_session_state_path =
            unique_test_path("auth-repository-sync-jobs-cross-org-duplicate-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let shared_repository_id = "repo_shared";
        let state = OrganizationState {
            organizations: vec![
                Organization {
                    id: "org_acme".into(),
            ...
            memberships: vec![OrganizationMembership {
                ...
                user_id: user_id.into(),
                role: OrganizationRole::Viewer,
                joined_at: "2026-04-21T01:00:00Z".into(),
            }],
            accounts: vec![LocalAccount {
                id: user_id.into(),
                email: "member@example.com".into(),
            ...
            repo_permissions: vec![
                RepositoryPermissionBinding {
                    organization_id: "org_acme".into(),
                    repository_id: shared_repository_id.into(),
                    synced_at: "2026-04-21T01:05:00Z".into(),
                },
                RepositoryPermissionBinding {
                    organization_id: "org_hidden".into(),
                    repository_id: shared_repository_id.into(),
                    synced_at: "2026-04-21T01:05:30Z".into(),
                },
            ],
            repository_sync_jobs: vec![
                sourcebot_models::RepositorySyncJob {
                    id: "sync_visible".into(),
                    organization_id: "org_acme".into(),
                    repository_id: shared_repository_id.into(),
                    connection_id: "conn_visible".into(),
                    status: sourcebot_models::RepositorySyncJobStatus::Succeeded,
                    queued_at: "2026-04-21T01:10:00Z".into(),
                ...
                sourcebot_models::RepositorySyncJob {
                    ...
                    connection_id: "conn_hidden".into(),
                    status: sourcebot_models::RepositorySyncJobStatus::Succeeded,
                    queued_at: "2026-04-21T01:11:00Z".into(),
        ...
        let payload: Vec<RepositorySyncJobListItemResponseBody> = read_json(response).await;
        assert_eq!(payload.len(), 1);
        assert_eq!(payload[0].id, "sync_visible");
        assert_eq!(payload[0].organization_id, "org_acme");
        assert_eq!(payload[0].repository_id, shared_repository_id);
    ...
    #[tokio::test]
    async fn auth_repository_sync_jobs_require_an_authenticated_local_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/repository-sync-jobs")
    ...
    async fn auth_oauth_clients_lists_only_clients_for_admin_visible_organizations_and_hides_secret_hash(
        ...
        let state = OrganizationState {
            ...
            oauth_clients: vec![
                OAuthClient {
                    id: "oauth_visible_active".into(),
                    organization_id: "org_acme".into(),
                    name: "Acme Web App".into(),
                    client_id: "client_visible_active".into(),
                    client_secret_hash: "$argon2id$v=19$m=19456,t=2,p=1$visible$active".into(),
                    redirect_uris: vec![
                        "https://acme.example.com/callback".into(),
                        "http://localhost:3000/callback".into(),
                    ],
                    created_by_user_id: user_id.into(),
                    created_at: "2026-04-24T00:05:00Z".into(),
                    revoked_at: None,
                },
                OAuthClient {
                    id: "oauth_visible_revoked".into(),
                    organization_id: "org_acme".into(),
                    name: "Acme CLI".into(),
                    client_id: "client_visible_revoked".into(),
                    client_secret_hash: "$argon2id$v=19$m=19456,t=2,p=1$visible$revoked".into(),
                    redirect_uris: vec![],
                    created_by_user_id: "local_user_admin".into(),
                    created_at: "2026-04-24T00:06:00Z".into(),
                    revoked_at: Some("2026-04-25T00:06:00Z".into()),
                },
                OAuthClient {
                    id: "oauth_hidden".into(),
                    organization_id: "org_hidden".into(),
                    name: "Hidden App".into(),
                    client_id: "client_hidden".into(),
                    client_secret_hash: "$argon2id$v=19$m=19456,t=2,p=1$hidden$app".into(),
                    redirect_uris: vec!["https://hidden.example.com/callback".into()],
                    created_by_user_id: "local_user_hidden".into(),
                    created_at: "2026-04-24T00:07:00Z".into(),
                    revoked_at: None,
                },
            ],
            ..OrganizationState::default()
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let payload: Vec<OAuthClientListItemResponseBody> = serde_json::from_slice(&body).unwrap();
        assert_eq!(payload.len(), 2);
        assert_eq!(payload[0].id, "oauth_visible_active");
        assert_eq!(payload[0].organization_id, "org_acme");
        assert_eq!(payload[0].name, "Acme Web App");
        assert_eq!(payload[0].client_id, "client_visible_active");
        assert_eq!(
            payload[0].redirect_uris,
            vec![
        ...
        assert_eq!(payload[0].created_at, "2026-04-24T00:05:00Z");
        assert_eq!(payload[0].revoked_at, None);
        assert_eq!(payload[1].id, "oauth_visible_revoked");
        assert_eq!(payload[1].organization_id, "org_acme");
        assert_eq!(payload[1].name, "Acme CLI");
        assert_eq!(payload[1].client_id, "client_visible_revoked");
        assert!(payload[1].redirect_uris.is_empty());
        assert_eq!(payload[1].created_by_user_id, "local_user_admin");
        assert_eq!(payload[1].created_at, "2026-04-24T00:06:00Z");
        assert_eq!(
            payload[1].revoked_at.as_deref(),
            Some("2026-04-25T00:06:00Z")
        );
    ...
    #[tokio::test]
    async fn auth_oauth_clients_require_an_authenticated_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/oauth-clients")
    ...
    #[tokio::test]
    async fn auth_connections_lists_durable_connections_for_organization_admin() {
        let organization_state_path = unique_test_path("auth-connections-orgs");
        let local_session_state_path = unique_test_path("auth-connections-sessions");
        let user_id = "local_user_admin";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        ...
        let state = OrganizationState {
            ...
            connections: expected_connections.clone(),
            memberships: vec![OrganizationMembership {
                organization_id: "org_acme".into(),
                user_id: user_id.into(),
                role: OrganizationRole::Admin,
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let payload: Vec<Connection> = serde_json::from_slice(&body).unwrap();
        assert_eq!(payload, expected_connections);
    ...
    #[tokio::test]
    async fn auth_connections_rejects_viewer_membership() {
        let organization_state_path = unique_test_path("auth-connections-viewer-orgs");
        let local_session_state_path = unique_test_path("auth-connections-viewer-sessions");
        let user_id = "local_user_viewer";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
    ...
    #[tokio::test]
    async fn auth_connections_require_an_authenticated_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/connections")
    ...
    #[tokio::test]
    async fn auth_connections_update_persists_trimmed_name_and_matching_config_without_changing_id()
    {
        let organization_state_path = unique_test_path("auth-connections-update-orgs");
        let local_session_state_path = unique_test_path("auth-connections-update-sessions");
        let user_id = "local_user_admin";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        ...
        let response = app
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&CreateConnectionRequestBody {
                            name: " Acme GitHub Updated ".into(),
                            kind: ConnectionKind::GitHub,
                            config: Some(ConnectionConfig::GitHub {
                                base_url: "https://github.enterprise.example.com".into(),
                            }),
                        })
                        .unwrap(),
                    ))
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let updated: Connection = serde_json::from_slice(&body).unwrap();
        assert_eq!(updated.id, existing_connection.id);
        assert_eq!(updated.name, "Acme GitHub Updated");
        assert_eq!(updated.kind, ConnectionKind::GitHub);
        assert_eq!(
            updated.config,
            Some(ConnectionConfig::GitHub {
                base_url: "https://github.enterprise.example.com".into(),
        ...
        let persisted: OrganizationState =
            serde_json::from_slice(&fs::read(&organization_state_path).unwrap()).unwrap();
        assert_eq!(persisted.connections, vec![updated.clone()]);
        assert_eq!(persisted.connections[0].id, existing_connection.id);
    ...
    #[tokio::test]
    async fn auth_connections_update_rejects_viewer_membership() {
        let organization_state_path = unique_test_path("auth-connections-update-viewer-orgs");
        let local_session_state_path = unique_test_path("auth-connections-update-viewer-sessions");
        let user_id = "local_user_viewer";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let existing_connection = Connection {
    ...
    #[tokio::test]
    async fn auth_connections_update_requires_an_authenticated_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .method("PUT")
    ...
    #[tokio::test]
    async fn auth_connections_update_rejects_blank_name_and_mismatched_config() {
        let organization_state_path = unique_test_path("auth-connections-update-invalid-orgs");
        let local_session_state_path = unique_test_path("auth-connections-update-invalid-sessions");
        let user_id = "local_user_admin";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let existing_connection = Connection {
    ...
    #[tokio::test]
    async fn auth_connections_update_rejects_missing_connection_id() {
        let organization_state_path = unique_test_path("auth-connections-update-missing-orgs");
        let local_session_state_path = unique_test_path("auth-connections-update-missing-sessions");
        let user_id = "local_user_admin";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let existing_connection = Connection {
        ...
        assert_eq!(invalid_payload_response.status(), StatusCode::NOT_FOUND);
    ...
    #[tokio::test]
    async fn auth_review_webhooks_detail_returns_visible_webhook_and_hides_secret_hash() {
        let organization_state_path = unique_test_path("auth-review-webhooks-detail-orgs");
        let local_session_state_path = unique_test_path("auth-review-webhooks-detail-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhooks/review_webhook_visible")
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
            )
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let payload: ReviewWebhookListItemResponseBody = serde_json::from_slice(&body).unwrap();
        assert_eq!(payload.id, "review_webhook_visible");
        ...
        assert_eq!(payload.created_at, "2026-04-25T00:05:00Z");
    ...
    #[tokio::test]
    async fn auth_review_webhooks_detail_requires_an_authenticated_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhooks/review_webhook_visible")
    ...
    #[tokio::test]
    async fn auth_review_webhooks_detail_returns_not_found_for_hidden_or_missing_webhooks() {
        let organization_state_path = unique_test_path("auth-review-webhooks-detail-missing-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-webhooks-detail-missing-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let hidden_response = app
            ...
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhooks/review_webhook_hidden")
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            )
        ...
        let missing_response = app
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhooks/review_webhook_missing")
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    #[tokio::test]
    async fn auth_review_webhook_delivery_attempts_list_only_attempts_for_visible_webhooks_and_exclude_hidden_or_orphaned_entries(
    ) {
        let organization_state_path =
            unique_test_path("auth-review-webhook-delivery-attempts-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-webhook-delivery-attempts-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let payload: Vec<ReviewWebhookDeliveryAttemptListItemResponseBody> =
            serde_json::from_slice(&body).unwrap();
        assert_eq!(payload.len(), 1);
        assert_eq!(payload[0].id, "delivery_attempt_visible");
    ...
    #[tokio::test]
    async fn auth_review_webhook_delivery_attempts_require_an_authenticated_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhook-delivery-attempts")
    ...
    #[tokio::test]
    async fn auth_review_webhook_delivery_attempts_for_webhook_detail_returns_visible_attempt() {
        let organization_state_path =
            unique_test_path("auth-review-webhook-delivery-attempts-for-webhook-detail-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-webhook-delivery-attempts-for-webhook-detail-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_visible/delivery-attempts/delivery_attempt_visible",
                    )
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let payload: ReviewWebhookDeliveryAttemptListItemResponseBody =
            serde_json::from_slice(&body).unwrap();
        assert_eq!(payload.id, "delivery_attempt_visible");
        assert_eq!(payload.webhook_id, "review_webhook_visible");
        assert_eq!(payload.connection_id, "conn_github_acme");
        assert_eq!(payload.repository_id, "repo_sourcebot_rewrite");
        ...
        assert_eq!(payload.accepted_at, "2026-04-25T00:10:00Z");
    ...
    #[tokio::test]
    async fn auth_review_webhook_delivery_attempts_for_webhook_detail_requires_an_authenticated_session(
    ) {
        let response = test_app()
            .oneshot(
                Request::builder()
    ...
    #[tokio::test]
    async fn auth_review_webhook_delivery_attempts_for_webhook_detail_returns_not_found_for_hidden_or_missing_webhooks_or_attempts(
    ) {
        let organization_state_path = unique_test_path(
            "auth-review-webhook-delivery-attempts-for-webhook-detail-missing-orgs",
        );
        let local_session_state_path = unique_test_path(
            "auth-review-webhook-delivery-attempts-for-webhook-detail-missing-sessions",
        );
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
            ...
            review_webhooks: vec![
                ...
                ReviewWebhook {
                    id: "review_webhook_other_visible".into(),
                    organization_id: "org_acme".into(),
                    connection_id: "conn_github_other".into(),
                    repository_id: "repo_other_visible".into(),
                    events: vec!["pull_request_review".into()],
                    secret_hash: "$argon2id$v=19$m=19456,t=2,p=1$other$hash".into(),
                    created_by_user_id: user_id.into(),
                    created_at: "2026-04-25T00:07:00Z".into(),
                },
            ],
            review_webhook_delivery_attempts: vec![
                ReviewWebhookDeliveryAttempt {
                ...
                ReviewWebhookDeliveryAttempt {
                    ...
                    repository_id: "repo_other_visible".into(),
                    event_type: "pull_request_review".into(),
                    review_id: "review_other_visible".into(),
                    external_event_id: "evt_other_visible".into(),
                    accepted_at: "2026-04-25T00:12:00Z".into(),
                },
            ],
            ..OrganizationState::default()
        ...
        let hidden_webhook_response = app
            .clone()
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_hidden/delivery-attempts/delivery_attempt_hidden",
                    )
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            ...
            .await
            .unwrap();
        assert_eq!(hidden_webhook_response.status(), StatusCode::NOT_FOUND);

        let missing_webhook_response = app
            .clone()
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_missing/delivery-attempts/delivery_attempt_visible",
                    )
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            ...
            .await
            .unwrap();
        assert_eq!(missing_webhook_response.status(), StatusCode::NOT_FOUND);

        let other_visible_attempt_response = app
            .clone()
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_visible/delivery-attempts/delivery_attempt_other_visible",
                    )
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            ...
            .unwrap();
        assert_eq!(
            other_visible_attempt_response.status(),
            StatusCode::NOT_FOUND
        );

        let missing_attempt_response = app
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_visible/delivery-attempts/delivery_attempt_missing",
                    )
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
            ...
            .await
            .unwrap();
        assert_eq!(missing_attempt_response.status(), StatusCode::NOT_FOUND);
    ...
    #[tokio::test]
    async fn auth_review_webhook_delivery_attempts_detail_returns_visible_attempt() {
        let organization_state_path =
            unique_test_path("auth-review-webhook-delivery-attempts-detail-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-webhook-delivery-attempts-detail-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhook-delivery-attempts/delivery_attempt_visible")
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    #[tokio::test]
    async fn auth_review_webhook_delivery_attempts_detail_requires_an_authenticated_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhook-delivery-attempts/delivery_attempt_visible")
    ...
    #[tokio::test]
    async fn auth_review_webhook_delivery_attempts_detail_returns_not_found_for_hidden_missing_or_orphaned_attempts(
    ) {
        let organization_state_path =
            unique_test_path("auth-review-webhook-delivery-attempts-detail-missing-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-webhook-delivery-attempts-detail-missing-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let hidden_response = app
            ...
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhook-delivery-attempts/delivery_attempt_hidden")
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            )
        ...
        let orphaned_response = app
            ...
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhook-delivery-attempts/delivery_attempt_orphaned")
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            )
        ...
        let missing_response = app
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhook-delivery-attempts/delivery_attempt_missing")
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    #[tokio::test]
    async fn auth_review_agent_runs_for_webhook_returns_only_runs_for_visible_webhook() {
        let organization_state_path = unique_test_path("auth-review-agent-runs-for-webhook-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-agent-runs-for-webhook-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
            ...
            review_agent_runs: vec![
                ReviewAgentRun {
                    id: "review_agent_run_visible_a".into(),
                    organization_id: "org_acme".into(),
                    webhook_id: "review_webhook_visible".into(),
                    delivery_attempt_id: "delivery_attempt_visible_a".into(),
                    connection_id: "conn_github_acme".into(),
                    repository_id: "repo_sourcebot_rewrite".into(),
                    review_id: "review_visible_a".into(),
                    status: ReviewAgentRunStatus::Queued,
                    created_at: "2026-04-25T00:15:00Z".into(),
                },
                ReviewAgentRun {
                    id: "review_agent_run_visible_b".into(),
                    organization_id: "org_acme".into(),
                    webhook_id: "review_webhook_visible".into(),
                    delivery_attempt_id: "delivery_attempt_visible_b".into(),
                    connection_id: "conn_github_acme".into(),
                    repository_id: "repo_sourcebot_rewrite".into(),
                    review_id: "review_visible_b".into(),
                    status: ReviewAgentRunStatus::Claimed,
                    created_at: "2026-04-25T00:16:00Z".into(),
                },
                ReviewAgentRun {
                    id: "review_agent_run_other_visible".into(),
                    organization_id: "org_acme".into(),
                    webhook_id: "review_webhook_other_visible".into(),
                    delivery_attempt_id: "delivery_attempt_other_visible".into(),
                    connection_id: "conn_github_acme_other".into(),
                    repository_id: "repo_other_visible".into(),
                    review_id: "review_other_visible".into(),
                ...
                ReviewAgentRun {
                    id: "review_agent_run_inconsistent_hidden_org".into(),
                    organization_id: "org_hidden".into(),
                    webhook_id: "review_webhook_visible".into(),
                    delivery_attempt_id: "delivery_attempt_inconsistent_hidden_org".into(),
                    connection_id: "conn_github_hidden".into(),
                    repository_id: "repo_private".into(),
                    review_id: "review_inconsistent_hidden_org".into(),
                    status: ReviewAgentRunStatus::Queued,
                    created_at: "2026-04-25T00:18:30Z".into(),
                },
                ReviewAgentRun {
                    id: "review_agent_run_orphaned".into(),
                    organization_id: "org_acme".into(),
                    webhook_id: "review_webhook_missing".into(),
                    ...
                    repository_id: "repo_orphaned".into(),
                    review_id: "review_orphaned".into(),
                    status: ReviewAgentRunStatus::Failed,
                    created_at: "2026-04-25T00:19:00Z".into(),
                },
            ],
            ..OrganizationState::default()
        };
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhooks/review_webhook_visible/review-agent-runs")
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
            )
        ...
        let payload: Vec<ReviewAgentRunListItemResponseBody> =
            serde_json::from_slice(&body).unwrap();
        assert_eq!(payload.len(), 2);
        assert_eq!(payload[0].id, "review_agent_run_visible_a");
        assert_eq!(payload[0].organization_id, "org_acme");
        assert_eq!(payload[0].webhook_id, "review_webhook_visible");
        assert_eq!(payload[0].delivery_attempt_id, "delivery_attempt_visible_a");
        assert_eq!(payload[0].connection_id, "conn_github_acme");
        assert_eq!(payload[0].repository_id, "repo_sourcebot_rewrite");
        assert_eq!(payload[0].review_id, "review_visible_a");
        assert_eq!(payload[0].status, "queued");
        assert_eq!(payload[0].created_at, "2026-04-25T00:15:00Z");
        assert_eq!(payload[1].id, "review_agent_run_visible_b");
        assert_eq!(payload[1].webhook_id, "review_webhook_visible");
        assert_eq!(payload[1].status, "claimed");
        assert_eq!(payload[1].created_at, "2026-04-25T00:16:00Z");
    ...
    #[tokio::test]
    async fn auth_review_agent_runs_for_webhook_requires_an_authenticated_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhooks/review_webhook_visible/review-agent-runs")
    ...
    #[tokio::test]
    async fn auth_review_agent_runs_for_webhook_returns_not_found_for_hidden_or_missing_webhooks() {
        let organization_state_path =
            unique_test_path("auth-review-agent-runs-for-webhook-missing-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-agent-runs-for-webhook-missing-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let hidden_response = app
            ...
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhooks/review_webhook_hidden/review-agent-runs")
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            )
        ...
        let missing_response = app
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-webhooks/review_webhook_missing/review-agent-runs")
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    #[tokio::test]
    async fn auth_review_agent_runs_for_webhook_detail_returns_visible_run() {
        let organization_state_path =
            unique_test_path("auth-review-agent-runs-for-webhook-detail-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-agent-runs-for-webhook-detail-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_visible/review-agent-runs/review_agent_run_visible",
                    )
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let payload: ReviewAgentRunListItemResponseBody = serde_json::from_slice(&body).unwrap();
        assert_eq!(payload.id, "review_agent_run_visible");
        assert_eq!(payload.organization_id, "org_acme");
        assert_eq!(payload.webhook_id, "review_webhook_visible");
        assert_eq!(payload.delivery_attempt_id, "delivery_attempt_visible");
        assert_eq!(payload.connection_id, "conn_github_acme");
        assert_eq!(payload.repository_id, "repo_sourcebot_rewrite");
        assert_eq!(payload.review_id, "review_visible");
        assert_eq!(payload.status, "queued");
        assert_eq!(payload.created_at, "2026-04-25T00:15:00Z");
    ...
    #[tokio::test]
    async fn auth_review_agent_runs_for_webhook_detail_requires_an_authenticated_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .uri(
    ...
    #[tokio::test]
    async fn auth_review_agent_runs_for_webhook_detail_returns_not_found_for_hidden_or_missing_webhooks_or_runs(
    ) {
        let organization_state_path =
            unique_test_path("auth-review-agent-runs-for-webhook-detail-missing-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-agent-runs-for-webhook-detail-missing-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
            ...
            review_agent_runs: vec![
                ...
                ReviewAgentRun {
                    ...
                    review_id: "review_inconsistent_hidden_org".into(),
                    status: ReviewAgentRunStatus::Queued,
                    created_at: "2026-04-25T00:16:30Z".into(),
                },
                ReviewAgentRun {
                    id: "review_agent_run_orphaned".into(),
                    organization_id: "org_acme".into(),
        ...
        let hidden_webhook_response = app
            ...
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_hidden/review-agent-runs/review_agent_run_hidden",
                    )
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
        ...
        let missing_webhook_response = app
            ...
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_missing/review-agent-runs/review_agent_run_visible",
                    )
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
        ...
        let hidden_run_response = app
            .clone()
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_visible/review-agent-runs/review_agent_run_hidden",
                    )
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            ...
            .await
            .unwrap();
        assert_eq!(hidden_run_response.status(), StatusCode::NOT_FOUND);

        let inconsistent_run_response = app
            .clone()
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_visible/review-agent-runs/review_agent_run_inconsistent_hidden_org",
                    )
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            ...
            .await
            .unwrap();
        assert_eq!(inconsistent_run_response.status(), StatusCode::NOT_FOUND);

        let orphaned_run_response = app
            .clone()
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_visible/review-agent-runs/review_agent_run_orphaned",
                    )
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            ...
            .await
            .unwrap();
        assert_eq!(orphaned_run_response.status(), StatusCode::NOT_FOUND);

        let missing_run_response = app
            .oneshot(
                Request::builder()
                    .uri(
                        "/api/v1/auth/review-webhooks/review_webhook_visible/review-agent-runs/review_agent_run_missing",
                    )
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
    ...
    #[tokio::test]
    async fn auth_review_agent_runs_detail_returns_visible_run() {
        let organization_state_path = unique_test_path("auth-review-agent-runs-detail-orgs");
        let local_session_state_path = unique_test_path("auth-review-agent-runs-detail-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-agent-runs/review_agent_run_visible")
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    #[tokio::test]
    async fn auth_review_agent_runs_detail_requires_an_authenticated_session() {
        let response = test_app()
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-agent-runs/review_agent_run_visible")
    ...
    #[tokio::test]
    async fn auth_review_agent_runs_detail_returns_not_found_for_hidden_or_missing_runs() {
        let organization_state_path =
            unique_test_path("auth-review-agent-runs-detail-missing-orgs");
        let local_session_state_path =
            unique_test_path("auth-review-agent-runs-detail-missing-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let state = OrganizationState {
        ...
        let hidden_response = app
            ...
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-agent-runs/review_agent_run_hidden")
                    .header(header::AUTHORIZATION, authorization.clone())
                    .body(Body::empty())
                    .unwrap(),
            )
        ...
        let missing_response = app
            .oneshot(
                Request::builder()
                    .uri("/api/v1/auth/review-agent-runs/review_agent_run_missing")
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
            )
    ...
    #[tokio::test]
    async fn review_webhook_events_persist_delivery_attempt_readable_via_authenticated_detail_endpoint(
    ) {
        let organization_state_path = unique_test_path("review-webhook-events-auth-detail-orgs");
        let local_session_state_path =
            unique_test_path("review-webhook-events-auth-detail-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let webhook_secret = "review-webhook-secret";
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri(format!(
                        "/api/v1/auth/review-webhooks/{}/delivery-attempts/{}",
                        delivery_attempt.webhook_id, delivery_attempt.id
                    ))
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
    ...
    #[tokio::test]
    async fn review_webhook_events_persist_delivery_attempt_readable_via_authenticated_listing_endpoint(
    ) {
        let organization_state_path = unique_test_path("review-webhook-events-auth-list-orgs");
        let local_session_state_path = unique_test_path("review-webhook-events-auth-list-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let webhook_secret = "review-webhook-secret";
        ...
        assert_eq!(payload.len(), 1);
        assert_eq!(payload[0].id, delivery_attempt.id);
        assert_eq!(payload[0].webhook_id, "review_webhook_1");
        assert_eq!(payload[0].connection_id, "conn_local");
        assert_eq!(payload[0].repository_id, "repo_sourcebot_rewrite");
        assert_eq!(payload[0].event_type, "pull_request_review");
        assert_eq!(payload[0].review_id, "review_123");
        assert_eq!(payload[0].external_event_id, "evt_123");
        assert_eq!(payload[0].accepted_at, delivery_attempt.accepted_at);
    ...
    async fn review_webhook_events_persist_review_agent_run_readable_via_authenticated_listing_endpoint(
        ...
        let local_session_state_path =
            unique_test_path("review-webhook-events-auth-run-list-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let webhook_secret = "review-webhook-secret";
        ...
        let review_agent_run = &persisted.review_agent_runs[0];
        assert_eq!(
            review_agent_run.delivery_attempt_id,
            persisted.review_webhook_delivery_attempts[0].id
        );
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri(format!(
                        "/api/v1/auth/review-webhooks/{}/review-agent-runs",
                        review_agent_run.webhook_id
                    ))
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let payload: Vec<ReviewAgentRunListItemResponseBody> = read_json(response).await;
        assert_eq!(payload.len(), 1);
        assert_eq!(payload[0].id, review_agent_run.id);
        assert_eq!(payload[0].organization_id, "org_acme");
        assert_eq!(payload[0].webhook_id, "review_webhook_1");
        assert_eq!(
            payload[0].delivery_attempt_id,
            review_agent_run.delivery_attempt_id
        );
        assert_eq!(payload[0].connection_id, "conn_local");
        assert_eq!(payload[0].repository_id, "repo_sourcebot_rewrite");
        assert_eq!(payload[0].review_id, "review_123");
        assert_eq!(payload[0].status, "queued");
        assert_eq!(payload[0].created_at, review_agent_run.created_at);
    ...
    #[tokio::test]
    async fn review_webhook_events_persist_review_agent_run_readable_via_authenticated_detail_endpoint(
    ) {
        let organization_state_path =
            unique_test_path("review-webhook-events-auth-run-detail-orgs");
        let local_session_state_path =
            unique_test_path("review-webhook-events-auth-run-detail-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let webhook_secret = "review-webhook-secret";
        ...
        let response = app
            .oneshot(
                Request::builder()
                    .uri(format!(
                        "/api/v1/auth/review-webhooks/{}/review-agent-runs/{}",
                        review_agent_run.webhook_id, review_agent_run.id
                    ))
                    .header(header::AUTHORIZATION, authorization)
                    .body(Body::empty())
                    .unwrap(),
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let payload: ReviewAgentRunListItemResponseBody = read_json(response).await;
        assert_eq!(payload.id, review_agent_run.id);
        assert_eq!(payload.organization_id, "org_acme");
        assert_eq!(payload.webhook_id, "review_webhook_1");
        assert_eq!(
        ...
        assert_eq!(payload.review_id, "review_123");
        assert_eq!(payload.status, "queued");
        assert_eq!(payload.created_at, review_agent_run.created_at);
    ...
    #[tokio::test]
    async fn review_webhook_events_persist_review_agent_run_readable_via_authenticated_run_listing_endpoint(
    ) {
        let organization_state_path =
            unique_test_path("review-webhook-events-auth-run-global-list-orgs");
        let local_session_state_path =
            unique_test_path("review-webhook-events-auth-run-global-list-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let webhook_secret = "review-webhook-secret";
    ...
    #[tokio::test]
    async fn review_webhook_events_persist_review_agent_run_readable_via_authenticated_run_detail_endpoint(
    ) {
        let organization_state_path =
            unique_test_path("review-webhook-events-auth-run-global-detail-orgs");
        let local_session_state_path =
            unique_test_path("review-webhook-events-auth-run-global-detail-sessions");
        let user_id = "local_user_member";
        let authorization =
            seed_local_session(&local_session_state_path.display().to_string(), user_id).await;
        let webhook_secret = "review-webhook-secret";
