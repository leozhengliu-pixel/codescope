...
8. Optional worker-only controls for the current baseline are:
   ...
   - `SOURCEBOT_WORKER_STATUS_PATH=<path>` (optional; preflights and writes a supervisor-readable JSON snapshot before execution, refreshes it after each bounded tick with `updated_at` and `process_id` heartbeat metadata, and writes it again after successful bounded completion with the last tick/outcome, nullable last work-item id/status/error, and nullable repository-sync terminal revision/branch/content-count metadata; control-character path values, existing directory targets, symlink targets/parents, or existing file-valued parents fail closed before startup so supervisor status cannot be silently misdirected)
