...
11. The worker still does **not** claim supervised workers, real fetch/import/reindex execution beyond the local-Git preflight plus persisted revision/content-count/current-branch and bounded tracked-content manifest/snapshot baseline, broad durable worker metadata beyond the shared state file, broad automated retry/backoff, or production scheduling/supervision; the bounded multi-tick loop is only a local/operator-controlled repeat-tick baseline, stale-running lease recovery is only a bounded queue recovery path before the next claim, and automatic retry is limited to one queued retry for stale-lease failures after a one-hour backoff when no active or prior automatic replacement exists.
