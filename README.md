...
9. `make sqlx-test` wraps the deterministic reset plus the focused `sourcebot-api` metadata storage, PostgreSQL-backed bootstrap-admin, durable local-session, PostgreSQL-backed local-account/membership/invite auth, and durable API-key/OAuth-client metadata regressions so local migration workflow verification uses one reproducible command.
10. `make sqlx-test` now covers the storage migration-inventory/catalog fallback checks, the PostgreSQL-backed bootstrap store and `/api/v1/auth/bootstrap` + login regressions, the PostgreSQL-backed local-session store and login -> `/api/v1/auth/me` regression slice, the durable PostgreSQL local-account lookup/linked-account membership/member-roster/invite-redeem regressions, plus PostgreSQL-backed API-key inventory/create/revoke/bearer-auth and OAuth-client inventory/create regressions; bootstrap-admin, invited-account login, auth-me identity restoration, member rosters, linked-account memberships, invite acceptance, API-key metadata, and OAuth-client metadata now all stay durable across API restarts when `DATABASE_URL` is configured.
...
2. Set `SOURCEBOT_DATA_DIR` in `.env` to the directory that should hold the current local runtime state. When only that shared base is set, the API and worker derive:
   ...
   - `local-sessions.json`
   - `organizations.json`
3. API-key inventory/create/revoke and OAuth-client inventory/create now restore from PostgreSQL-backed durable metadata when DATABASE_URL is configured. The same bounded auth slice also persists bootstrap-admin, local-session, local-account, membership, and invite-auth metadata durably in PostgreSQL for `/api/v1/auth/login`, `/api/v1/auth/me`, `/api/v1/auth/members`, `/api/v1/auth/linked-accounts`, `/api/v1/auth/invite-redeem`, `/api/v1/auth/api-keys`, and `/api/v1/auth/oauth-clients`; those routes ignore stale `bootstrap-state.json` / `local-sessions.json` / organization-auth fixture data for their PostgreSQL-backed reads, while connections, analytics, audit events, broader repo-permission aggregates, and the remaining whole-aggregate organization state still remain follow-up work.
