...
11. The worker still does **not** claim supervised workers, real fetch/import/reindex execution beyond the local-Git preflight plus persisted revision/content-count/current-branch and bounded tracked-content manifest/snapshot baseline, broad durable worker metadata beyond the shared state file, automated retry/backoff, scheduling, or continuous background orchestration; stale-running lease recovery is only a bounded one-shot queue recovery path before the next claim.
