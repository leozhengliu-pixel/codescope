...
help:
	@printf '%s\n' \
	  ...
	  'make sqlx-test - reset the deterministic test metadata database and run focused metadata plus durable catalog/local-import sync-job/bootstrap/local-account/repo-permission/repository-sync-job/review-agent-run/delivery-attempt/ask-thread/api-key/oauth-client auth tests' \
...
sqlx-test:
	...
	TEST_DATABASE_URL="$$TEST_DATABASE_URL" $(CARGO) test -p sourcebot-api --bin sourcebot-api auth_local_repository_import -- --nocapture --test-threads=1
	...
	TEST_DATABASE_URL="$$TEST_DATABASE_URL" $(CARGO) test -p sourcebot-api --bin sourcebot-api auth_repository_sync_jobs_ -- --nocapture --test-threads=1
	TEST_DATABASE_URL="$$TEST_DATABASE_URL" $(CARGO) test -p sourcebot-api pg_organization_store_ -- --nocapture --test-threads=1
	TEST_DATABASE_URL="$$TEST_DATABASE_URL" $(CARGO) test -p sourcebot-api --bin sourcebot-api pg_ask_thread_store_ -- --nocapture --test-threads=1
	...
	TEST_DATABASE_URL="$$TEST_DATABASE_URL" $(CARGO) test -p sourcebot-api --bin sourcebot-api postgres_metadata_when_database_url_is_configured -- --nocapture --test-threads=1
