...
require_line '^9\. `make sqlx-test` wraps the deterministic reset plus the focused `sourcebot-api` metadata storage, PostgreSQL-backed bootstrap-admin, durable local-session, and PostgreSQL-backed local-account/membership/invite auth regressions so local migration workflow verification uses one reproducible command\.$' 'truthful sqlx-test wrapper note'
require_line '^10\. `make sqlx-test` now covers the storage migration-inventory/catalog fallback checks, the PostgreSQL-backed bootstrap store and `/api/v1/auth/bootstrap` \+ login regressions, the PostgreSQL-backed local-session store and login -> `/api/v1/auth/me` regression slice, plus the durable PostgreSQL local-account lookup, linked-account membership, member-roster, and invite-redeem regressions; bootstrap-admin, invited-account login, auth-me identity restoration, member rosters, linked-account memberships, and invite acceptance now persist in PostgreSQL when `DATABASE_URL` is configured, while API keys, OAuth clients, connections, analytics, audit events, and the remaining whole-aggregate organization state remain later roadmap work\.$' 'truthful sqlx-test scope note'
