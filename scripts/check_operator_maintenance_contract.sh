...
require_line "$readme" '^2\. Record the runtime backup directory emitted by the helper; it contains copies of the current file-backed runtime paths plus a manifest\. When `DATABASE_URL` is configured, active durable local sessions live in PostgreSQL instead of `local-sessions\.json`, so pair runtime backups with the metadata backup flow below\.$' 'README runtime backup manifest note'
...
require_line "$restore_script" 'require_manifest_value' 'runtime restore manifest validation helper'
require_line "$restore_script" 'backup manifest .* does not match current' 'runtime restore manifest mismatch failure'
require_line "$acceptance_doc" 'validates that the backup manifest' 'operator maintenance runtime restore manifest validation contract'
require_line "$acceptance_doc" 'fails before copying any runtime files' 'operator maintenance runtime restore fail-before-copy contract'
...
require_line "$journeys_doc" 'manifest-validated runtime restore target checks' 'journeys runtime restore manifest validation reference'
...
require_line "$parity_report" 'validate the captured manifest' 'parity report runtime restore manifest validation evidence'
