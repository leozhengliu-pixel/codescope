...
require_line "$makefile" 'TEST_DATABASE_URL="\$\$TEST_DATABASE_URL" \$\(CARGO\) test -p sourcebot-api --bin sourcebot-api pg_ask_thread_store_ -- --nocapture --test-threads=1' 'Makefile pg ask-thread regression command'
...
require_line "$readme" '^9\. `make sqlx-test` wraps the deterministic reset plus the focused `sourcebot-api` metadata storage, PostgreSQL-backed catalog list/detail, PostgreSQL-backed bootstrap-admin, durable local-session, PostgreSQL-backed local-account/membership/invite auth, PostgreSQL-backed repository-permission filtering for authenticated sync-job history, PostgreSQL-backed repository-sync-job lifecycle storage, PostgreSQL-backed ask-thread/message storage, and durable API-key/OAuth-client metadata regressions so local migration workflow verification uses one reproducible command\.$' 'README sqlx-test wrapper note'
