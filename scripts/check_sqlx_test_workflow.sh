...
require_line "$makefile" 'make sqlx-test[[:space:]]+- reset the deterministic test metadata database and run focused metadata plus durable catalog/local-import sync-job/bootstrap/local-account/repo-permission/repository-sync-job/review-agent-run/delivery-attempt/ask-thread/api-key/oauth-client auth tests' 'Makefile help entry for sqlx-test'
