...
require_line "$makefile" 'make sqlx-test[[:space:]]+- reset the deterministic test metadata database and run focused metadata plus durable catalog/bootstrap/local-account/repo-permission/repository-sync-job/ask-thread/api-key/oauth-client auth tests' 'Makefile help entry for sqlx-test'
...
require_line "$readme" '^9\. `make sqlx-test` wraps the deterministic reset plus the focused `sourcebot-api` metadata storage, PostgreSQL-backed catalog list/detail, PostgreSQL-backed bootstrap-admin, durable local-session, PostgreSQL-backed local-account/membership/invite auth, PostgreSQL-backed repository-permission filtering for authenticated sync-job history, PostgreSQL-backed repository-sync-job lifecycle storage, PostgreSQL-backed ask-thread/message storage, and durable API-key/OAuth-client metadata regressions so local migration workflow verification uses one reproducible command\.$' 'README sqlx-test workflow note'
