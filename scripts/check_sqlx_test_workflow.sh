...
require_line "$makefile" 'make sqlx-test[[:space:]]+- reset the deterministic test metadata database and run focused metadata plus durable catalog/local-import/bootstrap/local-account/repo-permission/repository-sync-job/review-agent-run/delivery-attempt/ask-thread/api-key/oauth-client auth tests' 'Makefile help entry for sqlx-test'
...
require_line "$makefile" 'TEST_DATABASE_URL="\$\$TEST_DATABASE_URL" \$\(CARGO\) test -p sourcebot-api pg_organization_store_ -- --nocapture --test-threads=1' 'sqlx-test postgres organization-store command'
